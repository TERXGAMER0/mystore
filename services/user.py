...
class UserService:
    ...
    async def edit_language(telegram_id: int,
        ...
        if callback_data.language is None:
            msg = get_text(language, BotEntity.USER, "edit_language")
            for language_object in Language:
                kb_builder.button(
                    text=f"{language_object.get_flag_emoji()} {language_object.get_display_name()}",
            ...
            msg = get_text(language, BotEntity.USER, "edit_language_confirmation").format(
                current_language=user_dto.language.get_display_name(),
                update_language=callback_data.language.get_display_name()
            )
            kb_builder.button(
            ...
            await session_commit(session)
            msg = get_text(callback_data.language, BotEntity.USER, "language_edited_successfully")
